...
<body ng-controller="ShopMeCtrl" ng-init="loadProduct()">
<nav>
  <div class="container"> 
...
<div class="container" >
  <div class="row text-center">
  	<span><h3> {{order.Message}}</h3></span>
    <div class="" >
      <div class="thumbnail"> <img src="{{selectedProduct.imageUrl}}" alt="Thumbnail Image 1" class="img-responsive">
        <div class="caption">
          <h3>{{selectedProduct.name}}</h3>
          <p>{{selectedProduct.name}} - {{selectedProduct.color}} for {{selectedProduct.price}}</p>
          <div>
          <span> Quantity </span>
          	<select ng-model="selectedQuantity">
          		<option ng-repeat="qty in quantityDDP" value="{{qty}}">{{qty}}</option>
          	</select>
          	
          	<span>
          		<span>Your email address : </span><input type="text" ng-model="emailUser" class="style-1" required placeholder="Enter a valid email address"/> 
          	</span>
          </div>
          <p><a href="#" class="btn btn-primary" role="button" ng-click="createOrder(selectedProduct.id)"><span class="glyphicon glyphicon-shopping-cart" aria-hidden="true"></span> Buy </a></p>
        </div>
        <h3 class="text-center" >{{httpResponseMessage}}</h3>
        <div ng-show="noticeMeNeeded">
        <p><a href="#" class="btn btn-primary" role="button" ng-click="noticeMe(selectedProduct.id)"><span class="glyphicon glyphicon-shopping-cart" aria-hidden="true"></span> Notice Me...</a></p>
        </div>
      </div>
    </div>
    </div>
